...
- Узнайте побольше про file glob expansion с помощью `*` (а может еще и про `?` и `{`...`}`), кавычки, а так же разницу между двойными `"` и одинарными `'` кавычками. (Больше о расширении переменных читайте ниже)
