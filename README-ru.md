...
Продвинутому использованию командной строки зачастую не уделяют достаточного внимания, о терминале говорят, как о чём-то мистическом; на самом же деле &mdash; это умение прямо или косвенно увеличивает Вашу продуктивность в работе. Данный документ &mdash; это подборка заметок и советов, которые я нашёл для себя полезными, работая с командной строкой в Linux. Некоторые из них &mdash; простые и очевидные, но некоторые &mdash; довольно сложные и предназначены для решения конкретных задач. Это небольшая публикация, но если Вы знаете обо всём, что тут написано, и можете вспомнить, как это всё использовать &mdash; вы много знаете!
...
- Используйте `dmesg`, когда что-то ведёт себя совсем странно (например, железо или драйвера).
...
- `dmesg`: ошибки загрузки системы и ошибки системы
...
- Системы управлением пакетами &mdash; `brew` (Homebrew) и `port` (MacPorts). Они могут быть использованы для того, чтобы установить большинство програм, упомянутых в этом документе.
...
- Spotlight: Ищите файлы в консоли через `mdfind` и смотрите метаданные (например, EXIF – информацию из фотографий) через `mdls`.
