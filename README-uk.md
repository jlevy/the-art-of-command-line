...
- Дізнайтеся побільше про розширення шаблонів `*` (а також `?` і `[`...`]`), про вживання лапок та про відмінності між подвійними `"` та одинарними `'` лапками.
...
- Навчіться використовувати системами управління пакетами `apt-get`, `yum`, `dnf` та `pacman` (залежно від дистрибутива). Знайте як шукати і встановлювати пакети. Обов'язково майте встановленим `pip` для встановлення модулів та програм, написаних на Python (деякі з тих команд, що ви знайдете нижче, найлегше встановити через `pip`).
...
- Використовуйте `xargs` (або `parallel`). Це дуже корисно. Зверніть увагу, що ви можете контролювати кількість команд на кожен рядок (`-L`) також паралельність (`-P`). Якщо ви не впевнені, що робите щось правильно, почніть з `xargs echo`. Також `-I{}` - корисно. Приклади:
```bash
...
- Використовуйте `pgrep` і `pkill` для того, щоб знаходити або відправляти сигнали до процесів по імені (`-f` корисна опція).
- Знайте різні сигнали, які можна відправити процесам. Наприклад, щоб призупинити процес, використовуйте `kill -STOP [pid]`. Для повного списку подивіться `man 7 signal`.
...
- Збережіть свої синоніми (aliases), налаштування і функції які ви часто використовуєте у `~/.bashrc`, більш детально [тут](http://superuser.com/a/183980/7106). Це зробить їх доступними в усіх ваших сесіях shell.
...
- У Bash скриптах використовуйте `set -x` (або `set -v`) щоб дебажити вивід. Використовуйте строгий(strict) режим скрізь, де це можливо. Використовуйте `set -e` для того, щоб припиняти виконання при помилках (не нульовий вихідний код). Використовуйте `set -u` щоб визначити невстановленні змінні. Використовуйте `set -o pipefail` щоб при помилках не використовувати пайпи(вертикальна риска). Для більш складних скриптів також використовуйте `trap` на EXIT or ERR.
Корисною буде звичка почати використовувати наступний скрипт, який допоможе виявити і перервати на загальних помилках і вивести повідомлення:
```bash
      set -euo pipefail
      trap "echo 'error: Script failed: see failed command above'" ERR
```
- У Bash-скриптах підоболочки(subshells) - зручний спосіб сгрупувати команди. Один із найпоширеніших прикладів - тимчасово пересунутися в іншу робочу директорію:
```bash
...
- Фігурні скобки, використовуючи `{`...`}` може зменшити необхідність повторно вводити той самий текст і автоматизувати. Це корисно в такому прикладі як `mv foo.{txt,pdf} some-dir` (який пересуває обидва файла), `cp somefile{,.bak}` (який розширюється у `cp somefile somefile.bak`) або `mkdir -p test-{a,b,c}/subtest-{1,2,3}` (яка розширює всі можливі комбінації і створює дерево директорій).
...
- Розглянемо [`mosh`](https://mosh.mit.edu/) як альтернатива ssh який використовує UDP, уникаючи розривів підключення (вимагає установки на стороні сервера).
...
- Щоб швидко підняти веб-сервер в поточній директорії (і піддиректоріях), який доступний для всіх у вашій мережі, використовуйте:
...
- Якщо необхідно працювати з XML, є стара, але хороша утиліта - `xmlstarlet`.
...
- Ви можете встановити специфічне середовище для команди за допомогою префікса, а саме: `TZ=Pacific/Fiji date`.
...
- Для того, щоб стежити за пам'яттю використовуйте `free` і `vmstat`. Зокрема, не забувайте, що значення кешування ("cached" value) - Це пам'ять, яку тримає ядро і ці значення є частиною `free`.
