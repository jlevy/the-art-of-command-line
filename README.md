...
- The input or output of a command can be treated like a file via `>(some comand)` or `<(some command)` respectively. For example, compare local `/etc/hosts` with a remote one:
```sh
