...
- Learn at least one text-based editor well. Ideally Vim (`vi`), as there's really no competition for random editing in a terminal (even if you use Emacs, a big IDE, or a modern hipster editor most of the time). However, you should start editing with `nano` in order to get the idea of how you can open, close a file or even do a simple search while editing.
