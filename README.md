...
- Learn about `PATH` environment variable, you can examine it with `echo $PATH`. When you're executing a script file which name does not contain a slash, `PATH` is used to determine where the script is. Usually, the current directory is not in `PATH` for security reasons, so you need to use `./scriptname` or `sh scriptname` to execute a script in the current directory, make sure the script has execute permissions.
